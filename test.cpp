...
TEST(Data, NonConst) {
  String s = "abob";
  auto p = s.Data();
  bool are_same = std::is_same<decltype(s.Data()), const char *>();
...
TEST(SizeCapacity, IsUnsigned) {
  String s;
  EXPECT_TRUE(std::is_unsigned<decltype(s.Size())>());
  EXPECT_TRUE(std::is_unsigned<decltype(s.Capacity())>());
}
...
TEST(Multiply, Assignment) {
  String s = "a";
  EXPECT_TRUE((s *= 1000) == String(1000, 'a'));
}
...
TEST(Split, SomeTests) {
  ...
  }
  {
    ...
    EXPECT_TRUE(expected ==
                String("hello, world,no split here, , 1, ").Split(", "));
  }
  {
...
TEST(Join, Easy) { EXPECT_TRUE(String("aba") == String("b").Join({"a", "a"})); }
...
TEST(Join, Stress) {
  String a = String("a") * 1337;
  String b = String("b") * 1984;

  String c = String("c") * 228;
